...
export class NewsDO extends DurableObject<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    // 21 = Streak UTC migration (backfill last_signal_date from actual signal timestamps)
    ...
    if (appliedVersion < CURRENT_MIGRATION_VERSION) {
      ...
      // Run network-focus migration — reduces 17 beats to 10.
      // Removes external beats, remaps signals, preserves claims.
      // Single exec() call for atomic write coalescing.
      ...
      // Curation cleanup — fix Mar 28-29 inscription IDs and void orphaned earnings (#339).
      if (appliedVersion < 20) {
        for (const stmt of MIGRATION_CURATION_CLEANUP_SQL) {
          try {
            this.ctx.storage.sql.exec(stmt);
          } catch (e) {
            console.error("Curation cleanup migration failed:", e);
          }
        }
      }
